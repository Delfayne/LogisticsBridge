package com.tom.logisticsbridge.mixin.core;
...
public class MixinConfigPlugin implements IMixinConfigPlugin {
    ...
    private static final String PACKAGE_PREFIX = "com.tom.logisticsbridge.";
    ...
    private static final ImmutableMap<String, Consumer<PotentialMixin>> MIXIN_PROCESSING_MAP = ImmutableMap.<String, Consumer<PotentialMixin>>builder()
            ...
            .put("Lcom/tom/logisticsbridge/annotation/ClientOnlyMixin;", p -> p.isClientOnly = true)
            .put("Lcom/tom/logisticsbridge/annotation/LateMixin;", p -> p.isLate = true)
            .build();
    ...
    private void considerClass(String pathString) throws IOException {
        try(InputStream stream = MixinConfigPlugin.class.getClassLoader().getResourceAsStream("com/tom/logisticsbridge/mixin/" + pathString)) {
            if(stream == null)
                return;
    ...
    private static void writeOrderedProperties(Properties props, OutputStream stream) throws IOException {
        try(PrintWriter writer = new PrintWriter(stream)) {
            writer.println("# Logistics Bridge config file");
            writer.println();
    ...
    public void onLoad(String s) {
        if(allMixins.size() == 0) {
            try {
                URI uri = Objects.requireNonNull(MixinConfigPlugin.class.getResource("/mixins.logisticsbridge.json")).toURI();
                FileSystem fs;
                try {
            ...
            File targetConfig = new File(Launch.minecraftHome, "config" + File.separator + "logisticsbridge.properties");
            try {
    ...
    public List<String> getMixins() {
        ...
        if(phase == MixinEnvironment.Phase.DEFAULT) {
            ...
            List<String> list = allMixins.stream()
                    ...
                    .map(clz -> clz.replace("com.tom.logisticsbridge.mixin.", ""))
